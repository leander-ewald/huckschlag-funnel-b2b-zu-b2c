...
/* ══════════════════════════════════════════════════════════════
   OPTION DEFINITIONS — Echte Startup Funnel-Inhalte (gescrapt)
   ══════════════════════════════════════════════════════════════ */
...
export default function StartupFunnelPage() {
  ...
  const stepLabel = (): string => {
    if (step <= 4) return `Frage ${step + 1} von 6`;
    if (step === 5) return "Letzte Frage";
    return "Kontakt";
  };
      ...
      <div className="min-h-[100dvh] bg-gradient-to-br from-blue to-blue-dark flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl p-8 sm:p-12 max-w-lg w-full text-center">
          ...
          <p className="text-gray-600 mb-8">
            Wir melden uns innerhalb von <strong>24 Stunden</strong> bei dir &mdash; versprochen!
          </p>
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-6 sm:py-8">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden">
          <div className="p-4 sm:p-6 md:p-10">
            ...
            {step === 4 && <StepShop value={formData.shopSystem} onChange={(v) => updateField("shopSystem", v)} />}
              ...
              >
                {submitting ? (
                  ...
                  <>
                    ...
                    <span className="hidden sm:inline">Jetzt kostenloses Gespr&auml;ch sichern</span>
...
function StepDelivery({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <div>
      ...
      <p className="text-gray-400 text-sm mb-6">W&auml;hle die passende Gr&ouml;&szlig;enordnung.</p>
...
function StepProducts({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <div>
      ...
      <p className="text-gray-400 text-sm mb-6">Je genauer die Angabe, desto besser k&ouml;nnen wir euch beraten.</p>
...
function StepTarget({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <div>
      ...
      <p className="text-gray-400 text-sm mb-6">W&auml;hle eure Hauptzielgruppe.</p>
...
function StepShop({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <div>
      ...
      <p className="text-gray-400 text-sm mb-6">W&auml;hle euer aktuelles System.</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
...
function StepBudget({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  return (
    <div>
      ...
      <p className="text-gray-400 text-sm mb-6">Eine grobe Einsch&auml;tzung gen&uuml;gt.</p>
      <div className="grid grid-cols-2 gap-3">
...
function StepContact({ formData, updateField }: { formData: StartupFormData; updateField: <K extends keyof StartupFormData>(key: K, value: StartupFormData[K]) => void }) {
  ...
  return (
    <div>
      <p className="text-green text-xs font-semibold uppercase tracking-wider mb-1">Gro&szlig;artig! Klingt als k&ouml;nnten wir dir helfen</p>
